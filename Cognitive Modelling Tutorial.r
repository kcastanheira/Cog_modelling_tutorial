...
data = read.csv("~/desktop/Modelling Tutorial/Data.csv")
...
for(i in 1:(Nsubjects)){
    ...
    LL <- function(alpha, beta, mu) {
        ...
        #by assuming the estimated parameters should be close to 1
        -sum(log(Likelihood)) - log(dnorm(alpha, 1, 0.5)) - log(dnorm(beta, 1, 0.5)) - log(dnorm(mu, 1, 0.5))
    }
    ...
    for(k in 1:10){
        fit_param = tryCatch({ mle(LL, start = list(alpha=runif(1,0,1), beta=runif(1,0,1),
                                                mu=runif(1,0,1)))},
                                                error = function(e){
                                                               NULL})
    ...
    # If there is just return whatever estimate & LL was associated with a random instance
    ...
    #If NOT then just sva the minimum value
    else{
